...
const Sidebar = () => {
  ...
  const activeLiink = 'flex items-center gap-5 pl-4 pt-3 pb-2.5 rounded-lg text-white text-md m-2'
  const normalLink = 'flex items-center gap-5 pl-4 pt-3 pb-2.5 rounded-lg text-md text-gray-700 dark:text-gray-200 dark:hover:text-black hover:bg-light-gray m-2'

  return (
    <div className="ml-3 h-screen md:overflow-hidden overflow-auto 9
            ...
            {links.map((item) => (
              <div key={item.title}>
                ...
                {item.links.map((link) => (
                  ...
                  to={`/${link.name}`}
                  key={link.name}
                  className={({ isActive }) => isActive ? activeLiink : normalLink }
                  >
                    {link.icon}
